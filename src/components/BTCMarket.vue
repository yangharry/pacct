<template>
  ...
  <Market-listbtn @alignbtn="alignbtn($event)" :marketselect="2" />
      ...
      <tr v-for="(coin, i) in coins" :key="i">
        ...
        <td class="marketlist-cpytoprice">
          <strong
            ...
            >{{ Gettradeprice(coin.trade_price) }}</strong
          ><br />
          <span class="marketlist-subtext">{{
            Getcurrency(coin.acc_trade_price_24h)
          }}</span>
        </td>
        <td class="marketlist-cpytoror" :style="GetColor(coin.change)">
          <span class="marketlist-maintext custom1"
            >{{ Getpm(coin.change)
              ...
              Math.floor(coin.signed_change_price * krwbtc[0].trade_price)
            }}</span
          ><br />
          <span class="marketlist-maintext custom1"
            >{{ Getpm(coin.change)
    ...
    onMounted(async () => {
      await axios.get("https://api.upbit.com/v1/market/all").then((result) => {
        let newcoins = result.data.filter((result) => {
          return result.market.indexOf("BTC-") != -1;
        });
        coins.value = newcoins;
        coins2.value = newcoins;
        krwbtc.value = result.data.filter((result) => {
          return result.market.indexOf("KRW-BTC") != -1;
        });

        coins.value.forEach((result) => {
          symbols.value.push(result.market);
        });
        symbols.value.push(krwbtc.value[0].market);
        // console.log(symbols.value)
        // console.log(krwbtc.value[0].market)
      });
      ...
      ws.onmessage = async (result) => {
        ...
        if (krwbtc.value[0].market == ticker.code) {
          krwbtc.value[0] = Object.assign(krwbtc.value[0], ticker);
        } else if (coins.value[key] == null) {
          return;
        } else {
